...
@section('title-heading_2', 'جميع الشركات')
        ...
        <p>
            @if(session()->has('created_company_successfully'))
                <div class="alert alert-success text-center">
                    ...
                    {{ session()->get('created_company_successfully') }}
                </div>
            @elseif(session()->has('deleted_company_successfully'))
                <div class="alert alert-success text-center">
                    <a href="javascript:void(0);" class="close-btn text-decoration-none text-white" onclick="this.parentElement.style.display='none';" style="position:absolute; top:0px; right:5px; font-size: 150%;">&times;</a>
                    {{ session()->get('deleted_company_successfully') }}
                </div>
            @endif
        </p>
        ...
        <div class="row my-4">
          ...
          <div class="col-md-12">
            <div class="card shadow">
              <div class="card-body">
                <!-- table -->
                        <table class="table table-bordered border border-5 table-hover mb-0 @if($companies->count() == 0) d-none @endif">
                            ...
                            <tr>
                                ...
                                <td>{{ $company->description ?? 'لا يوجد' }}</td>
                                <td>{{ $company->created_at }}</td>
                                <td>{{ $company->updated_at ?? 'لا يوجد' }}</td>
                                <td>
                                    {{-- <a href="">edit</a>
                                    <a href="">delete</a> --}}
                        ...
                        <div class="alert alert-danger text-center">
                            <span class="h6">لا توجد شركات في قاعدة البيانات حتى الآن!</span>
                        </div>
                    @endforelse
                            </tbody>
                        </table>
