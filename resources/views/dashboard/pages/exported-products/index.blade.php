...
@section('title', 'جميع منصرف المخازن')
@section('title-heading_2', 'جميع منصرف المخازن')
@section('main-content')
<div class="container-fluid">
    <div class="row justify-content-center">
      <div class="col-12">
        <h2 class="mb-2 page-title">منصرف المخازن ({{ $exportedProducts->count() }})</h2>
        <p>
            @if(session()->has('created_exportedProduct_successfully'))
                <div class="alert alert-success text-center">
                    ...
                    {{ session()->get('created_exportedProduct_successfully') }}
                </div>
            @elseif(session()->has('deleted_exportedProduct_successfully'))
                <div class="alert alert-success text-center">
                    <a href="javascript:void(0);" class="close-btn text-decoration-none text-white" onclick="this.parentElement.style.display='none';" style="position:absolute; top:0px; right:5px; font-size: 150%;">&times;</a>
                    {{ session()->get('deleted_exportedProduct_successfully') }}
                </div>
            @endif
        </p>
              ...
              <div class="card-body">
                <!-- table -->
                        <table class="table table-bordered border border-5 table-hover mb-0 @if($exportedProducts->count() == 0) d-none @endif">
                            ...
                            {{-- @forelse ( $exportedProducts as $exportedProduct) --}}
                            <tr>
                                <td>{{ $exportedProduct->id }}</td>
                                <td>
                                    {{-- <div class="progress progress-sm" style="height:3px">
                                        <div class="progress-bar" role="progressbar" style="width: 87%" aria-valuenow="87" aria-valuemin="0" aria-valuemax="100"></div>
                                    </div> --}}
                                    {{ $exportedProduct->title }}
                                </td>
                                <td>{{ $exportedProduct->description ?? 'لا يوجد' }}</td>
                                <td>{{ $exportedProduct->amount }}</td>
                                <td>{{ $exportedProduct->created_at }}</td>
                                <td>{{ $exportedProduct->updated_at ?? 'لا يوجد' }}</td>
                                <td>
                                    {{-- <a href="">edit</a>
                                    <a href="">delete</a> --}}
                        ...
                        <div class="alert alert-danger text-center">
                            <span class="h6">لا يوجد منصرفات المخزن في قاعدة البيانات حتى الآن!</span>
                        </div>
                    @endforelse
                            </tbody>
                        </table>
