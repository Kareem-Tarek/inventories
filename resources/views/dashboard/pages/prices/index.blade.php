...
<div class="container-fluid">
    <div class="row justify-content-center">
      <div class="col-12">
        <h2 class="mb-2 page-title">الأسعار ({{ $prices->count() }})</h2>
        <p>
            @if(session()->has('created_price_successfully'))
                <div class="alert alert-success text-center">
                    ...
                    {{ session()->get('created_price_successfully') }}
                </div>
            @elseif(session()->has('deleted_price_successfully'))
                <div class="alert alert-success text-center">
                    <a href="javascript:void(0);" class="close-btn text-decoration-none text-white" onclick="this.parentElement.style.display='none';" style="position:absolute; top:0px; right:5px; font-size: 150%;">&times;</a>
                    {{ session()->get('deleted_price_successfully') }}
                </div>
            @endif
        </p>
        <div class="d-flex justify-content-end">
            <a href="{{ route('prices.create') }}" class="btn btn-primary">إضافة سعر</a>
        </div>
              ...
              <div class="card-body">
                <!-- table -->
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="progress progress-sm" style="height:3px">
                                        <div class="progress-bar" role="progressbar" style="width: {{ $price->discount * 100 }}%" aria-valuenow="{{ $price->discount * 100 }}" aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                </td>
                                <td>{{ $price->product->title ?? 'لا يوجد' }}</td>
                                <td>{{ $price->created_at }}</td>
                                <td>{{ $price->updated_at ?? 'لا يوجد' }}</td>
                                <td>
                                    {{-- <a href="">edit</a>
                                    <a href="">delete</a> --}}
                        ...
                        <div class="alert alert-danger text-center">
                            <span class="h6">لا توجد أسعار في قاعدة البيانات حتى الآن!</span>
                        </div>
                    @endforelse
                            </tbody>
                        </table>
